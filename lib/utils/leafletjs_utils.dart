...
abstract class PolylineUtil {
  ...
  static String encode(List<LatLng> points, [CodeOptions options]) {
    ...
    var output = _encode(points[0].latitude, 0.0, options.factor) + _encode(points[0].longitude, 0.0, options.factor);
    for (int i = 1; i < points.length; i++) {
      LatLng a = points[i];
      LatLng b = points[i - 1];
      output += _encode(a.latitude, b.latitude, options.factor);
      output += _encode(a.longitude, b.longitude, options.factor);
    }
    return output;
  }
